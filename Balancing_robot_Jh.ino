...
int acc_calibration_value = -8160;                            //Enter the accelerometer calibration value (IMPORTANT to find yours)
float pid_p_gain = 100;                                       //Gain setting for the P-controller (15)
float pid_i_gain = 15;                                      //Gain setting for the I-controller (1.5)
float pid_d_gain = 145;                                       //Gain setting for the D-controller (30)
...
void loop() {
  ...
  pid_p_gain = map(analogRead(A0), 0, 1028, 0, 300);
 pid_i_gain = map(analogRead(A1), 0, 1028, 0, 50);
  pid_d_gain = map(analogRead(A2), 0, 1028, 0, 300);
  ...
  Wire.beginTransmission(MPU);                                              //Start communication with the gyro
  Wire.write(0x43);                                                         //Start reading at register 45
  Wire.endTransmission();                                                   //End the transmission
  Wire.requestFrom(MPU, 2);                                                 //Request 2 bytes from the gyro
  ...
  if (pid_i_mem > 100)pid_i_mem = 100;                                      //Limit the I-controller to the maximum controller output
  else if (pid_i_mem < -100)pid_i_mem = -100;
  //Calculate the PID output value
  pid_output = pid_p_gain * pid_error_temp + pid_i_mem + pid_d_gain * (pid_error_temp - pid_last_d_error);
  if (pid_output > 100)pid_output = 100;                                    //Limit the PI-controller to the maximum controller output
...
ISR(TIMER2_COMPA_vect) {
  ...
  if (counter_motor > delayToBeAdded) {
    ...
    if (pid_output > 0) {
      ...
      PORTD |= 0b00000100; //Two different directions for the two steppers.
    }
    else {
      PORTD &= 0b00000000;
  ...
  else if (counter_motor == 1)PORTD |= 0b00011000;            //Set output 2 high to create a pulse for the stepper controller
  else if (counter_motor == 2)PORTD &= 0b11100111;            //Set output 2 low because the pulse only has to last for 20us
}
